...
# Penetration uses the RS-DAA HCV-antibody-positive eligible population.
rsdaa_antibody <- filter(antibody_positive, group == "Intervention")
penetration <- tibble(
  eligible_n = nrow(rsdaa_antibody),
  offered_n = sum(!is.na(rsdaa_antibody$`DAA Treatment Started`)),
  proportion = offered_n / eligible_n
)
readr::write_csv(penetration, file.path(OUTPUT_DIR, "implementation_penetration.csv"))

# Component-specific fidelity variables were not present in the supplied scripts.
# Add each prespecified indicator here only when its numerator and eligible
# denominator are explicitly identified in the raw intervention export.
